...
// PublicKeys is every key a release may be signed with. A rotation adds
// the new key here in a release signed by the old one; agents that have
// upgraded past it then accept releases signed by either, and the old
// key is dropped a few releases later.
var publicKeys = []string{PublicKey}

// TrustedKeys is a copy of the release keys the agent accepts.
func TrustedKeys() []string { return append([]string(nil), publicKeys...) }

// verifyAny accepts a signature by any of the trusted keys and returns
// the signed comment. The key is picked by the id inside the signature,
// so a signature never has to be tried against every key blindly.
func verifyAny(keys []string, sigFile, data []byte) (string, error) {
	...
	var last error
	for _, k := range keys {
		pk, err := parseMinisignKey(k)
		if err != nil {
			return "", err // a malformed trusted key is a build error, not a bad signature
		}
		if pk.id != s.id {
			continue
		}
		comment, err := verifyMinisign(k, sigFile, data)
		if err == nil {
			return comment, nil
		}
		last = err
	...
	return "", errors.New("signature: made with a key this agent does not trust")
}
