...
// preRelease is what may follow the dash: it ends up in a URL path.
var preRelease = regexp.MustCompile(`^[0-9A-Za-z.-]+$`)
...
func Parse(s string) (Version, error) {
	...
	if i := strings.IndexByte(s, '-'); i >= 0 {
		v.Pre = s[i+1:]
		s = s[:i]
		if v.Pre == "" || !preRelease.MatchString(v.Pre) {
			return Version{}, fmt.Errorf("invalid version %q: pre-release part", orig)
		}
	}
...
// String is the canonical form without a v prefix, the one the rest of
// the package passes around and writes down.
