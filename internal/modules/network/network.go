...
// Config is the `network:` section of agent.yml: exact interface names
// to watch, or glob patterns to leave out.
type Config struct {
...
// New is the network module's factory: it decodes the section, applies
// the default ignore list, and rejects a malformed glob up front.
...
// Collect adds per-interface rates once a previous reading exists — a
// counter that went backwards (the interface was reset) skips a tick —
// plus the socket counts.
