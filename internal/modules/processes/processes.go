...
// Config is the `processes:` section of agent.yml: how many top
// consumers to report (0 turns the list off) and which processes to watch.
type Config struct {
	Top   *int    `yaml:"top"`
	Watch []Watch `yaml:"watch"`
}

// Watch names a process to aggregate over: by exact comm, by a substring
// of its command line, optionally narrowed to one user.
type Watch struct {
...
// New is the processes module's factory: it decodes the section, bounds
// top to 0–100 and insists every watch names something to match.
...
// Collect walks /proc once: state and thread counts, each process's CPU
// since the last tick, then the top consumers and the watch aggregates.
